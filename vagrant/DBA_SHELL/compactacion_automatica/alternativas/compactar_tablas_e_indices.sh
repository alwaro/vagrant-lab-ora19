...
# QUIEN    CUANDO   QUE
#------- ---------  ------------------------------------------------------------------
# anaya  24-06-2019 Creacion del script 
# anaya  28-06-2019 Formateo de log + añadido estadisticas a la tabla compactada.
# anaya  09-07-2019 Creacion de informes finales
# ######################################################################################
...
export workDir=/scripts/oracle/dba/compactacion_automatica
export tablas_file=${workDir}/lista_tablas.txt
...
# ######################################################################################
#             BLOQUE CARGAR ENTORNO DE BBDD - VERSION EXADATA Y NO-EXADATA
# ######################################################################################
...
if [ $? -ne 0 ]; then
    ...
    if [[ "$hostPrefix" == "ex"?"-db0" ]]; then
        # Si esta corriendo, verificamos que tenga fichero de variables de entorno
        if [[ "$HOSTNAME" == "ex4-db01.des.int" ]] || [[ "$HOSTNAME" == "ex4-db02.des.int" ]]; then
            export ruta_entorno="/scripts/oracle/entorno/exadata-x4"
        elif [[ "$HOSTNAME" == "ex8-db01" ]] || [[ "$HOSTNAME" == "ex8-db02" ]]; then
            export ruta_entorno="/scripts/oracle/entorno/exadata-x8"
        else
        ...
        # Entorno NO EXADATA
        echo -e "\nSe ha detectado que no estamos en un entorno exadata.\n Esto implica que el ORACLE_HOME de la bbdd debe estar exportado antes de ejecutar el script\n\n"
        if [[ -z $ORACLE_HOME ]]; then
            msgQueue "CRITICAL" "Detectado host NO-exadata y no hay un ORACLE_HOME definido en memoria"
            exit 2;
        else
            msgQueue "DEBUG" "La variable ORACLE_HOME existe en memoria"
        fi
        
        if [[ "$ORACLE_SID" == "$database" ]]; then
            msgQueue "DEBUG" "La variable ORACLE_SID existe y su valor es igual que el argumento pasado al script: ${database}"
        else
            msgQueue "DEBUG" "Definiendo '${database}' como valor del ORACLE_SID"
